...
- “当前加载”指 `astmod`、`astredux` 或 `astflex` 的插件加载配置中至少有一处启用；AstRedux 继续复用大部分 AstMod 插件，但已用自己的 Challenge、Wave Spawner、AutoWipe adapter 和 Profile Controller 替换旧 DAS 链路。
