...
`optional/coop/ai_tank.smx` 统一负责 AI Tank 连跳、投石、原生翻越动画加速、机会性打铁和低障碍跳跃；`AI_HardSI` 继续负责其他特感，不再包含 Tank 模块，原 `tank_climb` 已合入。血量、伤害与击飞物规则仍由各自组件负责。

连跳按引擎当前追击目标距离停止追加跳跃，AstRedux 停止距离为 135；投石起手范围为 300–800，人数 profile 继续控制连跳与投石开关。机会性打铁只处理前方顺路、已在挥拳范围且与目标方向对齐的击飞物，不找铁绕路、不瞬间转向。低障碍跳跃要求前方追击路径受阻、顶面有支撑且碰撞体与分段弹道检查通过；失败时保留原生翻越，到顶后短暂检查支撑再继续追击。阈值和动作效果仍需试玩调整。

Hunter Skeet 的统一阈值由模式配置设为 150；四种狙击枪对 Hunter 的腹部额外倍率均由可重编译的 `optional/coop/l4d2_sniper_bodyshot.sp` 移除，保留传入伤害的其他修正与爆头规则。
