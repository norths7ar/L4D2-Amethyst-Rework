...
public void OnPluginStart() {
	// Cvars
	hCvarHardSIEnable = CreateConVar("ai_hardsi_enable", "1", "Enable the Hard SI behavior improvements.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	hCvarHardSIEnable.AddChangeHook(OnHardSIEnableChanged);
	// Event hooks
	...
	// Initialize module resources once, then apply the configured state.
	EnableHardSI();
	if (!hCvarHardSIEnable.BoolValue)
	{
		DisableHardSI();
	}
}
...
public void OnHardSIEnableChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	if (convar.BoolValue)
	{
		EnableHardSI();
...
{
	if (!g_bHardSIActive || g_hAssaultTimer != null)
	{
		return;
	}

	g_hAssaultTimer = CreateTimer(float(hCvarAssaultReminderInterval.IntValue), Timer_ForceInfectedAssault, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
