...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_fuck", Fuck_Cmd, ADMFLAG_BAN, "Slay the specified AI Special Infected. Supports partial name matching; all kills all infected players.");
...
public Action Fuck_Cmd(int client, int args)
{
	...
	if (StrEqual(inputArg, "all", false)) {
		...
		PrintToChatAll("\x01Sibalnoma \x03%N \x01fucked all special infected!", client);
	} else {
		int fuckCount = 0;
		for (int i = 1; i <= MaxClients; i++) {
			if ( IsClientInGame(i) && GetClientTeam(i) == TEAM_INFECTED && IsFakeClient(i) ) {
		...
		if (fuckCount) {
			PrintToChatAll("\x01Sibalnoma \x03%N \x01fucked %d %s!", client, fuckCount, inputArg);
		} else {
