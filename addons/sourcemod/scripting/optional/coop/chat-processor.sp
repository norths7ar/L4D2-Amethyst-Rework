...
public void OnPluginStart()
{
	...
	// Tags change presentation, not the original chat audience.
...
public Action OnSayText2(UserMsg msg_id, BfRead msg, const int[] players, int playersNum, bool reliable, bool init)
{
	...
	// With audience extensions disabled, keep the engine's exact recipients.
	// Reconstructing from team/dead flags can change map or plugin chat rules.
	if (!bDeadTalk && !bAllTalk && !bRestrictDeadChat)
	{
		for (int index = 0; index < playersNum; index++)
		{
			int recipient = players[index];
			if (recipient > 0 && IsClientInGame(recipient))
				recipients.Push(GetClientUserId(recipient));
		}
	}
	else for (int i = 1; i < MaxClients + 1; i++)
	{
