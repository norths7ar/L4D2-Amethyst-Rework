...
// Arg!'s 1.9 source from the maintainer-provided cannounce_1.9.zip.
// multicolors is sourced from the repository's AstMod scripting snapshot.
// Local addition: Anne join.sp-style early connecting notice; retain the
// original post-admin-check announcement and existing data/config formats.
#define VERSION "1.9-local"
...
new Handle:g_CvarShowConnecting = INVALID_HANDLE;
/*****************************************************************
...
public OnPluginStart()
{
	...
	g_CvarShowConnecting = CreateConVar("sm_ca_showconnecting", "1", "Announce when a human starts connecting, before authentication and loading complete.", _, true, 0.0, true, 1.0);
...
{
	if (GetConVarBool(g_CvarShowConnecting) && !IsFakeClient(client))
	{
		decl String:playerName[MAX_NAME_LENGTH];
		GetClientName(client, playerName, sizeof(playerName));
		CPrintToChatAll("%t", "CA Connecting", playerName);
	}
}
...
public OnClientPostAdminCheck(client)
{
	...
	if( GetConVarInt(g_CvarConnectDisplayType) == 1 )
	{
		...
		// Post-admin-check is a completed connection, including the final slot.
