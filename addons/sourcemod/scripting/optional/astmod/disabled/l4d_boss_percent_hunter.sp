...
public void OnMapStart()
{
	...
	if (StrEqual(g_sCurrentMap, g_sPrvMap) == false)
		g_iRoundCount = 1;

	g_iWitchFreq = GetConVarInt(FindConVar("l4d_multiwitch_spawnfreq"));
	g_iHunterLimit = GetConVarInt(FindConVar("z_hunter_limit"));
}
...
public void RoundEndEvent(Event event, const char[] name, bool dontBroadcast)
{
	GetCurrentMap(g_sPrvMap, sizeof(g_sPrvMap));
	g_iRoundCount++;
}

public void WitchFreqChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_iWitchFreq = GetConVarInt(FindConVar("l4d_multiwitch_spawnfreq"));
}

public void HunterLimitChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_iHunterLimit = GetConVarInt(FindConVar("z_hunter_limit"));
}
...
void PrintBossPercents(int client)
{
	...
	char message[512];
	char buffer[256];
	Format(message, sizeof(message), "\x01<\x05回合\x01> \x04%d    \x01<\x05Hunter\x01> \x04%d    \x01<\x05Current\x01> \x04%d%%%%\n", g_iRoundCount, g_iHunterLimit, boss_proximity);
	...
	if (!g_iWitchFreq) {
		...
		Format(buffer, sizeof(buffer), "\x01<\x05Witch\x01> \x04%i秒/个", g_iWitchFreq);
	}
	StrCat(message, sizeof(message), buffer);
