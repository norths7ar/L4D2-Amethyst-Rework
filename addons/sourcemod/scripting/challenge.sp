...
public Action OnPlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	// 击杀回血
	if (GetConVarBool(hRehealth) && isSurvivor(attacker)) {
		if (GetEntProp(attacker, Prop_Send, "m_isIncapacitated") != 0) return Plugin_Handled; // 濒死状态不加血
...
public Action OnInfectedDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	int attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
	if (attacker == 0 || victim == 0 || !isInfected(attacker)) return Plugin_Handled;
...
public Action OnTakeDamage(int victim, int& attacker, int& inflictor, float& damage, int& damagetype)
{
	...
	if ( isInfected(attacker) ) {
		...
		if ( zombie_class == ZC_SMOKER || zombie_class == ZC_HUNTER || zombie_class == ZC_JOCKEY || zombie_class == ZC_CHARGER )  { // 舌ht猴牛
