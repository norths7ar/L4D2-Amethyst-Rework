...
foreach ($path in @(
    'addons/astredux.vpk',
    'assets/astredux_vpk/scripts/gamemodes.txt',
...
foreach ($mode in $modes) {
    ...
    foreach ($group in @(
        @('optional/coop/ready_pause.smx', 'optional/pause.smx'),
        @('optional/coop/player_manager.smx', 'optional/playermanagement.smx'),
    ...
    foreach ($wave in @('optional/coop/wave_spawner.smx')) {
