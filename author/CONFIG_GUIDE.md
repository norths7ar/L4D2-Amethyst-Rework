...
`astredux` 复用同一条骨架，并以自己的 `cfg/cfgogl/astredux/`、`astredux` mutation、Redux 专属插件和 `addons/sourcemod/configs/astredux_profiles.cfg` 组成独立规则层。
...
- 历史命名、资产来源和已知限制记录在 `ASTMOD_INTEGRATION.md`。
